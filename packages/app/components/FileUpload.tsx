  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      if (response.pipeline) {
        ...
        // Initialize pipeline status with first step details
        // Note: Set status to 'running' since backend has already started pipeline
        setPipelineStatus({
          executionId: response.pipeline.executionId,
          status: response.pipeline.status,
          sanitizedName: response.pipeline.sanitizedName,
          steps: [
            {
              stepId: 'full_pipeline',
              status: 'running',
              name: 'Docling Hybrid Chunking',
              description: 'Converting PDF to semantic chunks with page tracking.\nPlease wait, this can take up to 10 minutes for large PDFs',
            },
          ],
        });
    ...
    const checkPipelineStatus = async () => {
      try {
        ...
        if (status.status === 'completed') {
          ...
          setMessage(`✗ Pipeline failed: ${status.error}`);
          setUploadStatus(UploadStatus.ERROR);
          onUploadError?.(`Pipeline failed: ${status.error}`);
        } else {
    ...
    <div className="flex flex-col items-center p-6 bg-white rounded-lg shadow-xl w-full max-w-md sm:max-w-lg md:max-w-xl">
          ...
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            ...
            >
              <path
                ...
                strokeWidth="2"
                d="M7 16a4 4 0 01-.88-7.903A5 0 0115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"
              ></path>
            </svg>
      ...
      {message && (
        <div className="w-full mt-4 p-3 rounded-md border">
          ...
          {uploadStatus === UploadStatus.SUCCESS && pipelineStatus && (
            <p className="text-xs text-blue-600 mt-2 flex items-center gap-2">
              ...
              Converting PDF using Docling Hybrid Chunking...
            </p>
          )}
        </div>
      )}
