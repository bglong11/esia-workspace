"""
FastAPI backend for ESIA Fact Extractor SaaS with Celery and Docling support
"""
...
import models
import schemas
from tasks import process_document_with_docling
...
app = FastAPI(
    title="ESIA Fact Extractor API (with Docling)",
    description="API for extracting facts from PDF documents using Docling and LLM",
    version="2.0.0"
)
...
# Path to your standalone docling script
DOCLING_SCRIPT_PATH = os.getenv(
    'DOCLING_SCRIPT_PATH',
    str(Path(__file__).parent.parent.parent / "docling_standalone.py")
)
...
async def root():
    ...
    return {"message": "ESIA Fact Extractor API (Docling)", "status": "running"}
...
async def upload_pdf(
    ...
    """
    Upload PDF file for processing with Docling

    Returns job information including job_id and celery task_id for tracking
    """
    # Validate file type
    ...
    task = process_document_with_docling.delay(
        job_id=job.id,
        file_path=str(file_path),
        docling_script_path=DOCLING_SCRIPT_PATH
    )

    # Store task ID for monitoring
    job.status = f"queued (task: {task.id})"
    db.commit()
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "service": "ESIA Fact Extractor API (Docling)",
        "celery_workers": worker_count,
        "docling_script": DOCLING_SCRIPT_PATH
    }
